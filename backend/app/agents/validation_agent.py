...
async def run_validation(state: AgentState) -> dict:
    ...
    affected_file = fusion.get("affected_file") or template.get("target_file") or "unknown"
    
    validation_group = state.get("validation", {})
    retry_count = validation_group.get("validation_retry_count", 0)
    ...
    try:
        ...
        if result.get("passed"):
            ...
            return {
                ...
                "workflow": {"current_step": "CREATING_MR"}
            }
